...
const PROJECT_CODEMETA = (await import(join(FIXTURES, "codemeta/project.json"))).default as Record<
  string,
  unknown
>;
const SPDX_CODEMETA = (await import(join(FIXTURES, "codemeta/spdx-license.json")))
  .default as Record<string, unknown>;
const MINIMAL_CODEMETA = (await import(join(FIXTURES, "codemeta/minimal.json"))).default as Record<
  string,
  unknown
>;
const PROJECT_CODEJSON = (await import(join(FIXTURES, "codejson/project.json"))).default as Record<
  string,
  unknown
>;
const ROUNDTRIP_INPUT = (await import(join(FIXTURES, "codejson/roundtrip-input.json")))
  .default as Record<string, unknown>;
...
describe("Integration: codemeta → code.json", () => {
  ...
  test("extracts license name from a valid SPDX URL", async () => {
    ...
    const licenses = (result.permissions as Record<string, unknown>).licenses as Array<
  ...
  test("sets license name to 'Other' when URL is not a recognised SPDX URL", async () => {
    ...
    expect(licenses[0]?.URL).toBe("https://github.com/DSACMS/codejson-crosswalk/blob/main/LICENSE");
  });
...
describe("Integration: code.json → codemeta", () => {
  ...
  test("uses the license URL from the first entry in permissions.licenses", async () => {
    const result = await fromCodejsonToCodemeta(PROJECT_CODEJSON);
    expect(result.license).toBe("https://github.com/DSACMS/codejson-crosswalk/blob/main/LICENSE");
  });
...
describe("Round-trip: codemeta → code.json → codemeta", () => {
  ...
  test("preserved: SPDX license URL survives the round-trip", async () => {
    const intermediate = await fromCodemetaToCodejson(SPDX_CODEMETA);
    const licenses = (intermediate.permissions as Record<string, unknown>).licenses as Array<
      Record<string, string>
    >;
    expect(licenses[0]?.name).toBe("MIT");
